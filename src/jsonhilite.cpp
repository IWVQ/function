...
void JSONHighlighter::colourise(int i, int t, int tk)
{
    if (t != i)
...
LBL_NUMBER:
    ...
    // spaces or eol
        ...
        colourise(f, i, JSON_SPACES); // don't store in token
    }
    // unknown
    else{
        token = JSON_DEFAULT;
        colourise(i, i + 1, token);
        i++;
    }
